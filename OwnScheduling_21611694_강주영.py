...
class Own_Algorithm:
    def __init__(self):
        ...
        self.quantum=dict() # time quantum
    ...
    # Quantum 간격 확인용
    def isQuantum(self,pID,Time):
        if (Time%self.quantum[pID])==0:
            return True
        else:
            return False

    def Own(self,ratio,Quantum,list_P):
        ...
        while True:
            while len(list_P) != 0:
                if self.runTime == list_P[0].arriveTime:
                    ...
                    if int(round(list_P[0].burstTime*ratio))<Quantum:
                        self.quantum[list_P[0].processID]=Quantum
                    else:
                        self.quantum[list_P[0].processID]=int(round(list_P[0].burstTime*ratio))
                    del list_P[0]
                else:
                    break
            ...
            if self.currentProcess != 0:
                ...
                elif self.isQuantum(runningProcess.processID,self.cpuTime-start_time):
                    list_E.append(self.runTime)
...
while(True):
    ...
    Menu = Tk()
    Menu.withdraw()
    ratio = askfloat("Ratio","원하는 Ratio을 입력하세요",minvalue=0.0001,maxvalue=1.0)
    Menu.destroy()
    ...
    RUNTIME,List_E,list_result = OWN.Own(ratio,Quantum,list_P)
